...
public sealed class MeasurementGenerator : IIncrementalGenerator {
    ...
    private const string AttrSource = @"// <auto-generated/>
...
";
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context) {
        context.RegisterPostInitializationOutput(ctx =>
            ctx.AddSource("MeasurementAttribute.g.cs", SourceText.From(AttrSource, Encoding.UTF8)));
        ...
        var types = context.SyntaxProvider.ForAttributeWithMetadataName(
            ...
            transform: static (ctx, _) => {
                ...
                var symbol = ctx.Attributes[0].ConstructorArguments.Length > 0
                    ...
                    : "";
                return (Name: type.Name, Namespace: type.ContainingNamespace.ToDisplayString(), Symbol: symbol);
            });

        context.RegisterSourceOutput(types, static (ctx, info) =>
            ctx.AddSource(info.Name + ".g.cs", SourceText.From(Emit(info.Name, info.Namespace, info.Symbol), Encoding.UTF8)));
    }

    private static string Emit(string name, string ns, string symbol) {
        string toStr = symbol.Length == 0 ? "$\"{value}\"" : $"$\"{{value}} {symbol}\"";
        string toStrFmt = symbol.Length == 0
            ? "value.ToString(format, provider)"
            : $"value.ToString(format, provider) + \" {symbol}\"";
...
public readonly partial struct {name}
    ...
    public bool NearlyEquals({name} other, int ulps = 4) {{
