...
public class Force {
    ...
    // SI units
    public static Force FromMeganewtons(double meganewtons) => new Force(meganewtons * 1e6);
    public double ToMeganewtons() => newtons / 1e6;
    public static Force FromKilonewtons(double kilonewtons) => new Force(kilonewtons * 1e3);
    ...
    public static Force FromMillinewtons(double millinewtons) => new Force(millinewtons * 1e-3);
    public double ToMillinewtons() => newtons / 1e-3;
    public static Force FromDynes(double dynes) => new Force(dynes * 1e-5);
    public double ToDynes() => newtons / 1e-5;

    // Gravitational & imperial units
    public static Force FromKilogramsForce(double kilogramsForce) => new Force(kilogramsForce * 9.80665);
    public double ToKilogramsForce() => newtons / 9.80665;
    public static Force FromPoundsForce(double poundsForce) => new Force(poundsForce * 4.4482216152605);
    public double ToPoundsForce() => newtons / 4.4482216152605;
    public static Force FromOuncesForce(double ouncesForce) => new Force(ouncesForce * 0.27801385095378125);
    public double ToOuncesForce() => newtons / 0.27801385095378125;
    public static Force FromPoundals(double poundals) => new Force(poundals * 0.138254954376);
