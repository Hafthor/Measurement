...
public class Area {
    ...
    // SI units
    public static Area FromSquareKilometers(double squareKilometers) => new Area(squareKilometers * 1e6);
    public double ToSquareKilometers() => squareMeters / 1e6;
    public static Area FromHectares(double hectares) => new Area(hectares * 1e4);
    public double ToHectares() => squareMeters / 1e4;
    public static Area FromAres(double ares) => new Area(ares * 1e2);
    public double ToAres() => squareMeters / 1e2;
    public static Area FromSquareMeters(double squareMeters) => new Area(squareMeters);
    public double ToSquareMeters() => squareMeters;
    public static Area FromSquareCentimeters(double squareCentimeters) => new Area(squareCentimeters * 1e-4);
    public double ToSquareCentimeters() => squareMeters / 1e-4;
    public static Area FromSquareMillimeters(double squareMillimeters) => new Area(squareMillimeters * 1e-6);
    public double ToSquareMillimeters() => squareMeters / 1e-6;

    // Imperial / US units
    public static Area FromSquareMiles(double squareMiles) => new Area(squareMiles * 2589988.110336);
    public double ToSquareMiles() => squareMeters / 2589988.110336;
    public static Area FromAcres(double acres) => new Area(acres * 4046.8564224);
    public double ToAcres() => squareMeters / 4046.8564224;
    public static Area FromSquareYards(double squareYards) => new Area(squareYards * 0.83612736);
    public double ToSquareYards() => squareMeters / 0.83612736;
    public static Area FromSquareFeet(double squareFeet) => new Area(squareFeet * 0.09290304);
    public double ToSquareFeet() => squareMeters / 0.09290304;
    public static Area FromSquareInches(double squareInches) => new Area(squareInches * 0.00064516);
    public double ToSquareInches() => squareMeters / 0.00064516;

    // Scientific units
    public static Area FromBarns(double barns) => new Area(barns * 1e-28);
