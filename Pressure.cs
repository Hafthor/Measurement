...
public class Pressure {
    ...
    // SI units
    public static Pressure FromMegapascals(double megapascals) => new Pressure(megapascals * 1e6);
    public double ToMegapascals() => pascals / 1e6;
    public static Pressure FromKilopascals(double kilopascals) => new Pressure(kilopascals * 1e3);
    public double ToKilopascals() => pascals / 1e3;
    public static Pressure FromHectopascals(double hectopascals) => new Pressure(hectopascals * 1e2);
    public double ToHectopascals() => pascals / 1e2;
    public static Pressure FromPascals(double pascals) => new Pressure(pascals);
    public double ToPascals() => pascals;

    // Bar units
    public static Pressure FromBars(double bars) => new Pressure(bars * 1e5);
    public double ToBars() => pascals / 1e5;
    public static Pressure FromMillibars(double millibars) => new Pressure(millibars * 1e2);
    public double ToMillibars() => pascals / 1e2;

    // Atmospheric & column units
    public static Pressure FromAtmospheres(double atmospheres) => new Pressure(atmospheres * 101325);
    public double ToAtmospheres() => pascals / 101325;
    public static Pressure FromTorr(double torr) => new Pressure(torr * 133.32236842105263);
    public double ToTorr() => pascals / 133.32236842105263;
    public static Pressure FromMillimetersOfMercury(double millimetersOfMercury) => new Pressure(millimetersOfMercury * 133.322387415);
    public double ToMillimetersOfMercury() => pascals / 133.322387415;
    public static Pressure FromInchesOfMercury(double inchesOfMercury) => new Pressure(inchesOfMercury * 3386.389);
    public double ToInchesOfMercury() => pascals / 3386.389;
    public static Pressure FromInchesOfWater(double inchesOfWater) => new Pressure(inchesOfWater * 249.08891);
    public double ToInchesOfWater() => pascals / 249.08891;

    // Imperial / US units
    public static Pressure FromPoundsPerSquareInch(double poundsPerSquareInch) => new Pressure(poundsPerSquareInch * 6894.757293168);
